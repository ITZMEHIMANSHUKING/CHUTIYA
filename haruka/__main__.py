...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("mishti.modules." + module_name)
...
def get_help(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 2 and any(args[1].lower() == x for x in HELPABLE):
            ...
            return
        update.effective_message.reply_text(
            "MERE SE PM ME MILO JAADI SE JAADI COMMAND KE LIYE🛠.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
def get_settings(update: Update, context: CallbackContext):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            text = "IDHAR CLICK KARO GROUP KI SETTING LANE KE LIYE🛠,YA APNI😛 ."
            msg.reply_text(
                text,
                reply_markup=InlineKeyboardMarkup(
                    [
            ...
            text = "IDHAR CLICK KARO APNI SETTING DEKHNE KE LIYE🙂."
...
def donate(update: Update, context: CallbackContext):
    ...
    if chat.type == "private":
        ...
        if OWNER_ID != 254318997 and DONATION_LINK:
            update.effective_message.reply_text(
                "TUM MERE OWNER KO DONATE KR SKTE HO BY CLICKING THE LINK "
        ...
        try:
            ...
            update.effective_message.reply_text(
                "KYA ME APKO PM KR SKTI HU DONATION KE LIYE"
            )
        except Unauthorized:
            update.effective_message.reply_text(
                "AGER MERE OWNER KO KHUCH DENA HAI TO DM ME AAO🥰."
            )
